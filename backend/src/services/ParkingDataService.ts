...
export class ParkingDataService {
  ...
  public async getParkingSpots(
    ...
    try {
      ...
      logger.info(`Returning ${allSpots.length} parking spots (${realSpots.length} real, ${demoSpots.length} demo)`);
      return allSpots;
    } catch (error) {
  ...
  private async getRealParkingSpots(
    location: { lat: number; lng: number },
    radius: number
  ): Promise<ParkingSpot[]> {
    try {
      ...
      return places.map((place: PlaceResult) => this.convertPlaceToParkingSpot(place));
    } catch (error) {
      logger.error('Error getting real parking spots:', error);
      return [];
    }
  }

  private async getDemoParkingSpots(
    location: { lat: number; lng: number },
    radius: number
  ): Promise<ParkingSpot[]> {
    try {
      ...
      logger.error('Error getting demo parking spots:', error);
      return [];
    }
  }

  private convertPlaceToParkingSpot(place: PlaceResult): ParkingSpot {
  ...
  private applyFilters(spots: ParkingSpot[], filters: any): ParkingSpot[] {
    let filtered = spots;
    ...
    if (filters.features && filters.features.length > 0) {
      filtered = filtered.filter(spot => 
        filters.features.every((feature: string) => spot.features.includes(feature))
      );
    }
  ...
  public async searchParkingSpots(
    query: string,
    location: { lat: number; lng: number }
  ): Promise<ParkingSpot[]> {
    try {
  ...
  public async getParkingSpot(id: string): Promise<ParkingSpot | null> {
    try {
